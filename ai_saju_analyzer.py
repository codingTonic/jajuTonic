...
class AISajuAnalyzer(LoggerMixin):
    def __init__(self, api_key: str = None):
        ...
        self.analysis_parts = {
            1: {
                ...
                "sections": [(1, "너는 어떤 코드로 이루어져 있을까? (너의 사주팔자)", 300)]
            },
            2: {
                "title": "'나'라는 존재 깊이 보기",
                "intro": "자, 이제 설계도를 바탕으로 '너'라는 사람의 본질을 깊이 들여다볼 시간이야. 너는 어떤 사람일까?",
                "sections": [
                    (2, "너라는 사람의 '본캐'는? (핵심 정체성)", 600),
                    (3, "네 안에 숨겨진 10가지 재능과 성향", 600),
                    (4, "너의 에너지 레벨과 스타일은?", 400)
                ]
            },
            3: {
                ...
                "sections": [
                    (5, "일할 때 너는 어떤 사람이야? (직업 & 사업)", 800),
                    (6, "너는 돈이랑 얼마나 친할까? (재물운)", 800),
                    (7, "사랑할 때 너는 어떤 모습이야? (연애 & 궁합)", 800)
                ]
            },
            4: {
                ...
                "sections": [
                    (8, "네 인생의 하이라이트는 언제일까? (인생 전체의 흐름)", 700),
                    (9, "올해 너에게 펼쳐질 이야기 (2025년)", 800)
                ]
            },
            5: {
                ...
                "sections": [
                    (10, "앞으로 1년, 월별 연애 & 금전 예보", 800),
                    (11, "가까운 미래, 어떤 준비를 해야 할까? (향후 3~5년)", 800)
                ]
            },
            6: {
                "title": "너를 위한 맞춤 생활 꿀팁",
                "intro": "사주 분석은 결국 더 잘 살기 위한 거잖아? 너의 일상에 바로 적용할 수 있는 소소하지만 강력한 팁들을 모아봤어.",
                "sections": [
                    (12, "네 인생의 '대박' 기회는 언제 올까?", 600),
                    (13, "인생의 파도를 지혜롭게 넘는 법", 600)
                ]
            },
            7: {
                "title": "너를 위한 맞춤 생활 꿀팁",
                "intro": "사주 분석은 결국 더 잘 살기 위한 거잖아? 너의 일상에 바로 적용할 수 있는 소소하지만 강력한 팁들을 모아봤어.",
                "sections": [
                    (14, "네 몸이 보내는 신호, 미리 챙기기 (건강 가이드)", 400),
                    (15, "너를 위한 '소울 푸드' 추천", 200),
                    (16, "너의 기운을 '충전'해주는 것들 (색, 방향, 장소)", 300)
                ]
            },
            8: {
                "title": "너의 이야기, 너의 힘",
                ...
                "sections": [
                    (17, "너라는 보석을 가장 빛나게 닦는 법 (종합 조언)", 800)
                ]
            }
        }

        # 파트별 참고 '패키지명'만 리스트로 관리
        self.part_package_mapping = {
            1: ["천간_지지_오행_기초", "60간지_특성_요약", "일주_이론_갑자_계해"],
            2: ["일간별_성격유형", "십성_기초_응용", "12운성_감정해석"],
            3: ["십성_기초_응용", "대운_직업운_흐름_사례_연구", "궁합궁위_음양비해석"],
            4: ["삼재_연도표", "운세_타이밍_프레임워크", "대운_직업운_흐름_사례_연구"],
            5: ["운세_타이밍_프레임워크", "운명_리딩_조언"],
            6: ["음양_오행_상생상극", "MBTI_오행_매칭표"],
            7: ["사주_리포트_템플릿", "실전_사주_작성스타일", "운명_리딩_조언"],
            8: ["실전_사주_작성스타일", "운명_리딩_조언"]
        }
    ...
    def _create_conversation_prompt(self, part_number: int, saju_result: Dict[str, Any], user_info: Dict[str, Any]) -> str:
        ...
        # 연애 상태 반영: 사랑/궁합 섹션일 때만 안내문 추가
        rel = user_info.get("relationship", "").lower()
        add_note = ""
        section_titles = [s[1] for s in part_info["sections"]]
        love_related = any(("연애" in title or "사랑" in title or "궁합" in title) for title in section_titles)
        if love_related:
            if rel in ["솔로", "single"]:
                add_note = "분석 시 현재 사용자는 솔로(싱글) 상태이므로, 연애의 시작, 새로운 인연, 소개팅 가능성, 이상형 등 '솔로' 관점에서 현실적이고 실용적으로 조언해줘."
            elif rel in ["연애중", "커플", "in_relationship"]:
                add_note = "분석 시 현재 사용자는 연애중이므로, 커플로서의 관계 유지, 소통, 갈등관리, 미래 전망 등 '연애중' 관점에서 구체적으로 분석해줘."
            elif rel in ["기혼", "married"]:
                add_note = "분석 시 현재 사용자는 결혼한 상태이므로, 부부 생활, 가족, 결혼생활의 변화와 안정, 배우자와의 궁합 등 '기혼' 관점에서 현실적으로 분석해줘."
            else:
                add_note = "분석 시 현재 사용자의 연애상태를 반드시 반영해 현실적으로 분석해줘."

        # sections_info 작성(추가 안내 포함)
        sections_info = "\n".join([
            f"- {s[1]} (최소 {s[2]}자)" + (f"\n  ※ {add_note}" if love_related and ("연애" in s[1] or "사랑" in s[1] or "궁합" in s[1]) else "")
